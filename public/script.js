
let navbarDiv = document.querySelector('.navbar');
window.addEventListener('scroll', () => {
    if(document.body.scrollTop > 40 || document.documentElement.scrollTop > 40){
        navbarDiv.classList.add('navbar-cng');
    } else {
        navbarDiv.classList.remove('navbar-cng');
    }
});


const navbarCollapseDiv = document.getElementById('navbar-collapse');
const navbarShowBtn = document.getElementById('navbar-show-btn');
const navbarCloseBtn = document.getElementById('navbar-close-btn');
// show navbar
navbarShowBtn.addEventListener('click', () => {
    navbarCollapseDiv.classList.add('navbar-collapse-rmw');
});

// hide side bar
navbarCloseBtn.addEventListener('click', () => {
    navbarCollapseDiv.classList.remove('navbar-collapse-rmw');
});

document.addEventListener('click', (e) => {
    if(e.target.id != "navbar-collapse" && e.target.id != "navbar-show-btn" && e.target.parentElement.id != "navbar-show-btn"){
        navbarCollapseDiv.classList.remove('navbar-collapse-rmw');
    }
});

// stop transition and animatino during window resizing
let resizeTimer;
window.addEventListener('resize', () => {
    document.body.classList.add("resize-animation-stopper");
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(() => {
        document.body.classList.remove("resize-animation-stopper");
    }, 400);
});

// var dropdowns = document.getElementsByClassName("dropdown");
// for (var i = 0; i < dropdowns.length; i++) {
//   dropdowns[i].addEventListener("touchstart", function() {
//     this.classList.toggle("hover");
//   });
// }

const cancel = document.querySelectorAll('.cbtn')
for(let i=0;i<cancel.length;i++){
cancel[i].addEventListener('click',function(){
        let a = cancel[i].parentElement.parentElement.querySelector("h3").innerText
        let b = a.split(":")[1];
        if(confirm("Do you want to delete")){
        let Details = {
            place : b
        }
        fetch("/remove",
                {
                    method: "POST",
                    headers: {
                        'Accept': 'application/json, text/plain, */*',
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify(Details)
                })
                cancel[i].parentElement.parentElement.remove()
            }
})
}


// const block = document.querySelectorAll('.button')

// for(let i=0;i<block.length;i++){
// block[i].addEventListener('click',function(){
//         let b = block[i].parentElement.parentElement.parentElement.querySelector("h3").innerText
//         if(confirm("Do you want to delete")){
//         let Details = {
//             email : b
//         }
//         fetch("/removeuser",
//                 {
//                     method: "POST",
//                     headers: {
//                         'Accept': 'application/json, text/plain, */*',
//                         'Content-Type': 'application/json'
//                     },
//                     body: JSON.stringify(Details)
//                 })
//                 // block[i].parentElement.parentElement.remove()
//             }
// })
// }

